...
class CarlaDataCollector:
    ...
    def spawn_traffic(self, num_vehicles=100, num_pedestrians=50):
        ...
        for i in range(num_vehicles):
            ...
            if random.random() < 0.8:
                ...
                blueprint = random.choice(bus_bps) if bus_bps else random.choice(car_bps)
    ...
    def save_frame_data(self, rgb_image, seg_image, annotations, weather_condition):
        ...
        seg_path = os.path.join(self.output_dir, weather_condition, 'segmentation', f"{frame_name}.png")
        cv2.imwrite(seg_path, seg_vis)
        
        # Save annotations as JSON
        ann_path = os.path.join(self.output_dir, weather_condition, 'annotations', f"{frame_name}.json")
    ...
    def save_yolo_annotation(self, annotations, weather_condition, frame_name):
        ...
        yolo_path = os.path.join(self.output_dir, weather_condition, 'annotations', f"{frame_name}.txt")
    ...
    def generate_dataset_analysis(self):
        ...
        for weather, stats in self.dataset_stats['weather_conditions'].items():
            summary_stats['weather_distribution'][weather] = {
                ...
                'objects_per_frame': stats['total_objects'] / stats['frames_collected'] if stats['frames_collected'] > 0 else 0
            }
    ...
    def _create_visualizations(self, analysis_dir, summary_stats):
        ...
        weather_frames = [stats['frames'] for stats in summary_stats['weather_distribution'].values()]
        weather_labels = list(summary_stats['weather_distribution'].keys())
        
        ax1.bar(weather_labels, weather_frames, color=['gold', 'navy', 'skyblue', 'gray'])
        ax1.set_title('Frames per Weather Condition', fontsize=14, fontweight='bold')
        ax1.set_ylabel('Number of Frames')
        ax1.grid(axis='y', alpha=0.3)
        
        # Objects per weather condition
        weather_objects = [stats['objects'] for stats in summary_stats['weather_distribution'].values()]
        ax2.bar(weather_labels, weather_objects, color=['gold', 'navy', 'skyblue', 'gray'])
        ...
        ax3.pie(class_counts, labels=classes, autopct='%1.1f%%', colors=colors, startangle=90)
        ax3.set_title('Overall Class Distribution', fontsize=14, fontweight='bold')
        
        # Objects per frame by weather
        weather_obj_per_frame = [summary_stats['weather_distribution'][w]['objects_per_frame'] 
                                for w in weather_labels]
        ax4.bar(weather_labels, weather_obj_per_frame, color=['gold', 'navy', 'skyblue', 'gray'])
        ax4.set_title('Average Objects per Frame by Weather', fontsize=14, fontweight='bold')
        ...
        for i, weather in enumerate(weather_conditions):
            ...
            for j, class_name in enumerate(class_names):
                class_matrix[i, j] = weather_data['objects_per_class'].get(class_name, 0)
        ...
        for j, class_name in enumerate(class_names):
            ax.bar(weather_conditions, class_matrix[:, j], bottom=bottom, 
                   label=class_name, color=colors[j], alpha=0.8)
            bottom += class_matrix[:, j]
        
        ax.set_title('Class Distribution by Weather Condition', fontsize=16, fontweight='bold')
        ax.set_ylabel('Number of Instances')
        ax.legend()
        ax.grid(axis='y', alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(os.path.join(analysis_dir, 'class_distribution_by_weather.png'), 
        ...
        class_percentages = [count/total_instances*100 for count in summary_stats['class_distribution'].values()]
        ...
        ax2.bar(weather_labels, weather_percentages, color=['gold', 'navy', 'skyblue', 'gray'])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(analysis_dir, 'dataset_balance.png'), dpi=300, bbox_inches='tight')
        plt.close()
    
    def _generate_analysis_report(self, analysis_dir, summary_stats):
        """Generate detailed analysis report"""
        report_path = os.path.join(analysis_dir, 'dataset_analysis_report.md')
        ...
        with open(report_path, 'w',encoding="UTF-8") as f:
            ...
            f.write(f"- **Total Frames:** {overview['total_frames']:,}\n")
            f.write(f"- **Total Annotated Instances:** {overview['total_instances']:,}\n")
            f.write(f"- **Weather Conditions:** {overview['weather_conditions']}\n")
            f.write(f"- **Object Classes:** {overview['object_classes']}\n")
            f.write(f"- **Average Instances per Frame:** {overview['total_instances']/overview['total_frames']:.2f}\n\n")
            ...
            for weather, stats in summary_stats['weather_distribution'].items():
                f.write(f"| {weather.capitalize()} | {stats['frames']:,} | {stats['objects']:,} | {stats['objects_per_frame']:.2f} |\n")
            ...
            for class_name, count in summary_stats['class_distribution'].items():
                ...
                f.write(f"| {class_name.replace('_', ' ').title()} | {count:,} | {percentage:.1f}% |\n")
            
            # Detailed Weather Analysis
            f.write("\n## Detailed Weather Analysis\n\n")
            for weather, detailed_stats in summary_stats['detailed_weather_stats'].items():
                f.write(f"### {weather.capitalize()} Conditions\n")
                f.write(f"- **Frames Collected:** {detailed_stats['frames_collected']:,}\n")
                f.write(f"- **Total Objects:** {detailed_stats['total_objects']:,}\n")
                f.write(f"- **Average Objects per Frame:** {detailed_stats['total_objects']/detailed_stats['frames_collected']:.2f}\n")
                f.write("- **Class Distribution:**\n")
                
                for class_name, count in detailed_stats['objects_per_class'].items():
            ...
            f.write(f"- **Balance Ratio:** {balance_ratio:.3f} (1.0 = perfect balance)\n")
            f.write(f"- **Min Class Count:** {min_class:,}\n")
            f.write(f"- **Max Class Count:** {max_class:,}\n")
            ...
            weather_counts = [stats['frames'] for stats in summary_stats['weather_distribution'].values()]
            min_weather = min(weather_counts)
            max_weather = max(weather_counts)
            weather_balance = min_weather / max_weather if max_weather > 0 else 0
            
            f.write(f"\n### Weather Condition Balance\n")
            f.write(f"- **Balance Ratio:** {weather_balance:.3f}\n")
            f.write(f"- **Min Weather Frames:** {min_weather:,}\n")
            f.write(f"- **Max Weather Frames:** {max_weather:,}\n")
            ...
            f.write(f"- **Total Training Samples:** {overview['total_frames']:,}\n")
            
            if overview['total_frames'] > 8000:
                f.write("- **Assessment:** Excellent sample size for deep learning ✓\n")
            elif overview['total_frames'] > 4000:
                f.write("- **Assessment:** Good sample size for deep learning ✓\n")
            elif overview['total_frames'] > 2000:
                f.write("- **Assessment:** Moderate sample size, augmentation recommended ⚠️\n")
            else:
                f.write("- **Assessment:** Small sample size, extensive augmentation required ❌\n")
            ...
            if balance_ratio < 0.5:
                ...
                f.write("- Use **targeted augmentation** for underrepresented classes\n")
            
            f.write("- **Recommended augmentations:**\n")
            f.write("  - Rotation (±15°)\n")
            f.write("  - Horizontal flipping\n")
            f.write("  - Brightness adjustment (±20%)\n")
            f.write("  - Contrast adjustment (±15%)\n")
            f.write("  - Gaussian noise addition\n")
            f.write("  - Random cropping with resize\n")
            
            f.write("\n### Training Strategy\n")
            f.write("- **Dataset Split Recommendation:**\n")
            f.write("  - Training: 70% (stratified by weather and class)\n")
            f.write("  - Validation: 20%\n")
            f.write("  - Testing: 10%\n")
            f.write("- **Cross-validation:** Consider weather-stratified k-fold\n")
            f.write("- **Evaluation:** Test each model on each weather condition separately\n")
            
            f.write("\n---\n")
            f.write("*This analysis provides the foundation for YOLOv11 model training and evaluation.*\n")
...
class DatasetAnalyzer:
    ...
    def analyze_dataset_statistics(self):
        ...
        for weather, weather_annotations in annotations.items():
            ...
            if weather_data['frame_count'] > 0:
                weather_data['avg_objects_per_frame'] = weather_data['total_objects'] / weather_data['frame_count']
            
            stats['weather_stats'][weather] = weather_data
            stats['object_density'].extend([len(ann.get('objects', [])) for ann in weather_annotations])
    ...
    def create_advanced_visualizations(self, stats, output_dir):
        ...
        ax2.hist(stats['aspect_ratios'], bins=50, alpha=0.7, color='lightcoral', edgecolor='black')
        ...
        ax3.hist(stats['object_density'], bins=30, alpha=0.7, color='lightgreen', edgecolor='black')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'advanced_dataset_analysis.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        for i, (weather, ax) in enumerate(zip(weather_conditions, axes.flat)):
            ...
            if counts:  # Only plot if there's data
                ...
                ax.set_title(f'{weather.capitalize()} - Class Distribution', fontweight='bold')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'weather_specific_analysis.png'), 
                   dpi=300, bbox_inches='tight')
        plt.close()
        ...
        bars = ax1.bar(weather_names, avg_objects, 
                      color=['gold', 'navy', 'skyblue', 'gray'], alpha=0.8)
        ...
        for bar, avg in zip(bars, avg_objects):
            height = bar.get_height()
            ax1.text(bar.get_x() + bar.get_width()/2., height + max(avg_objects)*0.01,
...
def main():
    ...
    try:
        ...
        total_frames = sum([data['frame_count'] for data in stats['weather_stats'].values()])
